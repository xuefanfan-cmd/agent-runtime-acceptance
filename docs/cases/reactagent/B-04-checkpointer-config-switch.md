...
com.huawei.ascend.sit.cases.component.singleagent.TwoTurnDialogueRunner
  run(mainplanClient, logPrefix) → Result
  （文案/超时常量在 Runner 内；Turn2 语义断言由 Runner 内部调用）
```
...
读取方式：文案与超时写在 `TwoTurnDialogueRunner` 常量（B-03 / B-04 共用）；JSON 仅外置参考。**不**使用 `main` ScenarioData。
...
void inMemoryThenRedis_sameDialogue_bothPassSemantics() {
    ...
    try (SutStack p1 = buildInMemoryStack().start()) {
        TwoTurnDialogueRunner.run(p1.client("mainplan"), "B-04.P1");
    }

    try (SutStack p2 = buildRedisStack(redisUrl).start()) {
        TwoTurnDialogueRunner.run(p2.client("mainplan"), "B-04.P2");
    }
}
```
