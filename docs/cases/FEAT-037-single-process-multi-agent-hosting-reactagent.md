...
以 `multi-react-travel-hosted-demo` 的一个 JVM 进程和其中两个真实 ReactAgent 为黑盒 SUT，验证 FEAT-037 的注册、发现、路由、默认实例、错误、协议兼容、Runtime Task/会话隔离、Core 共享会话清理边界、生命周期、共享额度、活动任务查询和单实例兼容。两个实例的注册 ID 固定为 `agent-a`、`agent-b`，业务回答由确定性 OpenAI 兼容端点回显实例身份与唯一 canary；测试只断言公开 HTTP/A2A/SSE、Card、Task、活动任务快照和进程日志。
...
- 测试类为 `ReactAgentMultiInstanceHostingBlackboxTest`；Txx 映射方法 `txx<ScenarioName>`、Story `FEAT-037.Txx: <场景>`、tag `story-feat-037-txx`、DisplayName `FEAT-037 Txx ReactAgent <场景>`，矩阵 33 行均已实现为唯一方法。
...
.\.agents\skills\feature-acceptance-testing\scripts\run-wsl-tests.ps1 `
  -TestSelector "ReactAgentMultiInstanceHostingBlackboxTest#t15UnknownInstanceReturnsProtocolErrorWithoutCatalog,ReactAgentMultiInstanceHostingBlackboxTest#t17ResponsePreservesProtocolWithoutHostedMetadata,ReactAgentMultiInstanceHostingBlackboxTest#t26SharedSessionResetKeepsTargetRouting,ReactAgentMultiInstanceHostingBlackboxTest#t30WrongInstanceUsesTaskNotFoundWithoutOwnerLookup" `
