...
```json
{
  "_doc": "B-03 / B-04 Redis checkpointer two-turn dialogue. Authority: TwoTurnDialogueRunner constants.",
  "turn1Text": "我要出差",
  "turn2Text": "2026年6月19日从深圳去北京出差3天，给我推荐酒店，没有任何要求，随机选一个就好",
...
读取方式：文案与超时写在 `TwoTurnDialogueRunner` 常量；JSON 仅外置参考。**不**使用 `main` ScenarioData。
