...
**构造算法**（`cases/.../boundary/LongMessageInputBuilder`）：
...
读取方式：模板/填充/超时写在 `LongTravelMessageFlow` 常量；由 Builder 拼出长文（**禁止**在测试方法内硬编码 5000 字全文）。JSON 仅外置参考。**不**使用 `main` ScenarioData。
...
- [ ] C-07.*.E：后置「你好」硬断言
- [ ] 超时写在 `LongTravelMessageFlow` 常量（JSON 仅参考）
- [ ] 不进 SmokeTestSuite
