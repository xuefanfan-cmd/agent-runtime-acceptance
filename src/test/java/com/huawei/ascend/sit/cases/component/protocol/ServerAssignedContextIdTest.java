 ...
 * </ul>
 *
 * <p>SUT 默认走 {@code sut.base.url}（{@code application-local.yml} 配为 7.209.189.82:13003，
 * 指向预部署的 mainplan）。本用例不依赖 SUT 业务输出——只要任一终态（COMPLETED / INPUT_REQUIRED
 * 等 {@link TaskState#isFinal()}=true）即可观测协议契约，所以无论 mainplan + LLM 是否走到追问
 * 分支，断言都成立。
 *
...
@Tag("component")
@Tag("smoke")
class ServerAssignedContextIdTest extends BaseComponentTest {
    ...
    void serverAssignsContextIdAndPreservesItAcrossTurns() {
        ...
        Message turn1 = Message.builder()
                ...
                .build();
        TurnObservation t1 = sendOneTurn(turn1);
        ...
        Message turn2 = Message.builder()
                ...
                .parts(List.of(new TextPart(INPUT_TEXT)))
                .build();
        TurnObservation t2 = sendOneTurn(turn2);
    ...
    private TurnObservation sendOneTurn(Message message) {
        ...
        a2aClient.sendMessage(message, null, consumers, errorHandler);
