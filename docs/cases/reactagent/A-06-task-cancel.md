...
smoke_scope: 纳入 SmokeTestSuite（@Tag("smoke")）；流式 + 同步两条子场景均纳入；执行前由测试人员保证 LLM 与 mainplan 就绪
---
...
| 3 | mainplan 进程已具备可用 LLM 配置（`LLM_*` 环境变量、`sut.java.system-properties` 等，见 §7） | 由测试人员启动前准备；缺失时 send/cancel 可能 **FAIL**（超时无 taskId、终态 `FAILED`、cancel 窗口错过等） |
| 4 | **无需** trip / hotel | — |
| 5 | 流式 / 同步子场景各用 **独立测试类**（`streaming` 栈级配置互斥，见 §9） | — |
...
| # | 动作 | 协议 / 方法 | 预期 |
|---|------|------------|------|
| 1 | 拉起 mainplan（`streaming(true)`） | `SutStack` | 就绪；LLM 由启动环境注入（见 §7） |
| 2 | 使用测试常量 `INPUT_TEXT`（长 prompt） | — | 非空 |
...
| # | 动作 | 协议 / 方法 | 预期 |
|---|------|------------|------|
| 1～2 | 同 A-06-S，但栈为 `streaming(false)` | — | — |
...
- **Given**：传输门禁（A-06.S.0 / A-06.Y.0）已通过；mainplan 已按 §7 配置 LLM；collector 已收到含 `taskId` 的事件。
...
A-06 **不在测试代码中**读取 `SIT_LLM_API_KEY`、不使用 `@EnabledIf` 按 profile 跳过。执行前由测试人员保证 mainplan 能调用 LLM，方式与 A-04 / A-05 / `StreamingTravelPlanningTest` 对齐：
...
常用变量（按实际 SUT 与网关要求配置；长 prompt 场景需模型能进入 `WORKING` 态）：
...
未配置或配置错误时，不在测试侧提前 FAIL；表现为 A-06.*.A（超时无 taskId / cancel 窗口错过）、A-06.*.B/C（cancel/get 失败）或 A-06.S.D（流式终态非 `CANCELED`）等断言失败。
...
- 若在 `WORKING` 下发 cancel 且 A-06.*.B/C 均为 `CANCELED` → 视为 LLM 探针 **PASS**。
...
读取方式：文案与超时写在 `AgentTaskCancelStreamTest` 常量（A-06-S / A-06-Y 共用）。JSON 仅外置参考。**不**使用 `main` ScenarioData。
...
| 栈 | `AgentTaskCancelStreamTest.mainplanStack(config, true)` → `.streaming(true).agent("mainplan")` | `mainplanStack(config, false)` → `.streaming(false).agent("mainplan")` |
| send | 虚拟线程：`sendMessage` + `A2aEventCollector` | **后台线程**阻塞 send + 主线程 cancel |
| cancel / get | `TaskCancelVerifiers.assertCancelAndGet(...)` | 同左 |
| smoke 范围 | `@Tag("smoke")`；无 `@EnabledIf`；执行前由测试人员配置 LLM（§7） | 同左 |

> **为何两个测试类**：`BaseManagedStackTest` 在 `@BeforeAll` 按类固定 `streaming` 标志；同一类内无法交替 `true`/`false` 而不手动重建 Client。

**实现检查清单**：

- [ ] 显式 `.streaming(true)` / `.streaming(false)`（经共享 `mainplanStack`）
...
# 流式子场景（推荐先跑）
./mvnw -Dtest=AgentTaskCancelStreamTest test

# 同步子场景
./mvnw -Dtest=AgentTaskCancelSyncTest test

# 两条一起
./mvnw -Dtest=AgentTaskCancelStreamTest,AgentTaskCancelSyncTest test

# 指定 profile（可选，影响 application-<env>.yml）
./mvnw -Dtest.env=LOCAL -Dtest=AgentTaskCancelStreamTest test

# smoke
./mvnw -Dtest=SmokeTestSuite test
```

> 缺 LLM 时测试仍会执行并在 cancel 窗口 / cancel-get 断言处失败；执行前请按 §7 自行配置凭据与代理。
...
| 特性 3-2 流式（前置） | S | A-06.S.0 / S.D | ⚠️ 传输前置 |
| 特性 3-1 同步（前置） | Y | A-06.Y.0 | ⚠️ 传输前置 |
...
- **同步子场景 taskId 观测**：阻塞 `message/send` 路径下，collector 可能在 `taskIdWaitMs` 内收不到含 `taskId` 的事件（与流式路径行为不同）；失败时优先核对 A-06-Y 的事件推送时序，必要时调大 `taskIdWaitMs` 或调整 send 并发模型。
