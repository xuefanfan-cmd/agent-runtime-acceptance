...
> 续轮不篡改、且两轮 `taskId` 互异。
>
> **关键定位**：OJ-01 验证「能通」；OJ-02 验证「会话 ID 协议契约」。与 A-11-2
> （`ServerAssignedContextIdTest`）同构，仅换 openjiuwen 制品（`-Dtest.env=openjiuwen`）。
...
| 2 | Turn 1：`Message.builder()...` **无 contextId** | `A2aServiceClient.sendMessage` + `A2aEventCollector` | 终态 `isFinal()` |
| 3 | 记录 Turn 1 的 `contextId`、`taskId` | collector | 均非空 |
| 4 | Turn 2：同样文本，**显式** `.contextId(turn1ContextId)` | 同上 | 终态 `isFinal()` |
...
| 客户端 | **不**走 `InteractionFlow`；直接 `A2aServiceClient` + `A2aEventCollector`（同 A-11-2） |
| 参考实现 | `ServerAssignedContextIdTest` |
