...
class ReactAgentHostedRemoteCatalogBlackboxTest {
    ...
    private HostedRemoteCatalogBlackboxSupport driver() {
        return new HostedRemoteCatalogBlackboxSupport(config, HOSTED_AGENT, false, model);
    }

    @Test
    @Tag("story-feat-037-t36")
    @Story("FEAT-037.T36: 远端目录继承、追加和覆盖闭环")
    @DisplayName("FEAT-037 T36 ReactAgent 远端目录按实例继承追加并完整覆盖")
    ...
    @DisplayName("FEAT-037 T37 ReactAgent 局部发现失败不回退全局且独立恢复")
    ...
    @DisplayName("FEAT-037 T38 ReactAgent 全局后续发现传播且不覆盖局部同名目标")
    ...
    @DisplayName("FEAT-037 T40 ReactAgent 同名同 URL 的 TLS Client 按实例目录隔离")
    ...
    @DisplayName("FEAT-037 T41 ReactAgent 非法局部远端配置在 ready 前失败")
    ...
    @DisplayName("FEAT-037 T42 ReactAgent 旧式 Caller 拒绝实例局部远端目录")
    ...
    @DisplayName("FEAT-037 T43 ReactAgent 旧式 Caller 保持全局远端调用兼容")
