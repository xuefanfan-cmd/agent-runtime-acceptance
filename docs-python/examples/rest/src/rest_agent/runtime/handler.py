...
**不依赖任何外部服务**——无模型、无网络、无存储。这让契约档用例可重复执行：
同一请求任意次数得到同一响应（host_app.py 的 `_FixtureHandler` 同构）。

固定事件序列取 docs/examples 与 host_app 的形态：thought → tool_start → 终答。
终答作为**内容帧** `final_answer_chunk` 投递，成功完成由流正常结束表达
（compatibility.md 已知兼容边界 #1：不得臆造 COMPLETED chunk）。
"""
