---
title: 装配与配置片段（snippets）
description: 叠加能力的配置与接线增量，供 how-to 页面引用并并入已有 Agent 服务工程
audience: ai-coding
---

# 装配与配置片段（snippets）

本目录存放**单文件增量片段**：中间件、Sandbox 沙箱、SkillHub、Custom REST，以及 Runner 基础托管所需的
通用 YAML。Agent 类型自身的完整框架源码集放在 [../examples/](../examples/)，不在此重复。

| | examples/ | snippets/（本目录） |
| --- | --- | --- |
| 形态 | 完整源码用例（Application + Configuration + 配套类 + yml，不含 pom） | 单文件片段（一个类 / 一段 yml） |
...
3. 每个片段至少被一篇 how-to 引用，how-to 必须说明它是新增、替换还是合并。

## 片段索引

| 仓内片段 | 复制到工程时的目标文件名 | 内容 | 引用它的 how-to |
| --- | --- | --- | --- |
