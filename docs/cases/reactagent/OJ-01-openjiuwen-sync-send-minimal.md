...
>
> 单 agent 启动时 YAML 为 `remote-agents[0]` 提供占位 URL，避免 discovery NPE；全链用例经
> `SutStack.downstream(...)` 覆盖为真实下游地址。
...
| 3 | `InteractionFlow.of(client).send("你好").awaitState(COMPLETED).assertTask(...)` | main `InteractionFlow` | 阻塞至终态；文本非空 |
| 4 | 断言 OJ-01.A / B | — | 见 §4 |
...
- **When**：发送「你好」，等待终态。
- **Then**：`TaskState == TASK_STATE_COMPLETED`（SDK 枚举）。
...
- **When**：经统一 `TaskTextExtractor.textOf(task)` 抽取 artifact 文本。
- **Then**：文本非空。
...
外置参考：`src/test/resources/testdata/component/protocol/oj-01-sync-send.json`
...
超时取自 `sut.timeout.poll-seconds`（`TestConfig`），**不**使用 `main` 下的 ScenarioData record；
用例常量与上述 JSON 对齐即可。
...
| 栈描述 | `SutStack.builder(config).streaming(false).agent("mainplan")` |
| 配置 | `-Dtest.env=openjiuwen` → `application-openjiuwen.yml` |
| 驱动 | `InteractionFlow`（`awaitState` + `assertTask`） |
| 文本抽取 | `TaskTextExtractor.textOf` |
| 断言 | AssertJ：`COMPLETED`、`isNotBlank(text)` |

> **不**引入 `OpenjiuwenStackSupport` / `model.openjiuwen.*`；制品差异只在 YAML profile。
...
- **与 spring-ai-ascend A-03 的差异**：SUT 为 openjiuwen travel mainplan 制品；断言勿硬编码 spring-ai-ascend Card 字段。
